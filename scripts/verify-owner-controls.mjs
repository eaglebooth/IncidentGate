...
const [targetStatus, gateStats, gateVersion] = await Promise.all([
  read(target, "get_status"),
  read(gate, "get_stats"),
  read(gate, "get_contract_version"),
]);

process.stdout.write(`${JSON.stringify({ targetStatus, gateStats, gateVersion }, null, 2)}\n`);
